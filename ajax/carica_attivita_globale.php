...
// === ANIME ===
$sql_anime = "SELECT u.username, u.profile_image, aa.titolo, aa.riferimento_api, aa.episodi_visti, aa.status, aa.anno_uscita, aa.formato
    ...
    while ($riga = $result_anime->fetch_assoc()) {
        ...
        if ($id_api > 0) {
            ...
            if ($response !== false) {
                ...
                // Prendiamo la descrizione dall'API
                if (isset($data['data']['Media']['description'])) {
                    $descrizione = strip_tags($data['data']['Media']['description']);
                    // Limita la descrizione a circa 200 caratteri
        ...
        $attivita[] = [
            ...
            "profile_image" => $riga["profile_image"],
            "titolo" => $riga["titolo"],
...
// === MANGA ===
$sql_manga = "SELECT u.username, u.profile_image, am.titolo, am.riferimento_api, am.capitoli_letti, am.status, am.anno, am.formato
...
$sql_fumetti = "SELECT u.username, u.profile_image, af.titolo, af.riferimento_api, af.numero_letti, af.status, af.anno_uscita, af.nome_volume, af.numero_fumetto
...
if ($result_fumetti) {
    while ($riga = $result_fumetti->fetch_assoc()) {
        ...
        if ($id_api > 0) {
            $url = "https://comicvine.gamespot.com/api/issue/4000-" . $id_api . "/?api_key=" . $api_key . "&format=json&field_list=image,description";
            $opts = [
                "http" => [
            ...
            if ($response !== false) {
                ...
                // Prendiamo la descrizione dall'API
                if (isset($data["results"]["description"])) {
                    $descrizione = strip_tags($data["results"]["description"]);
                    // Limita la descrizione a circa 200 caratteri
                    if (strlen($descrizione) > 200) {
                        $descrizione = substr($descrizione, 0, 197) . '...';
                    }
...
$sql_videogiochi = "SELECT u.username, u.profile_image, av.titolo, av.guid, av.ore_giocate, av.status, av.data_uscita
...
if ($result_videogiochi) {
    while ($riga = $result_videogiochi->fetch_assoc()) {
        ...
        if (!empty($guid)) {
            $url = "https://www.giantbomb.com/api/game/" . $guid . "/?api_key=" . $api_key_videogiochi . "&format=json&field_list=image,deck";
            $opts = [
                "http" => [
            ...
            if ($response !== false) {
                ...
                // Prendiamo la descrizione dall'API
                if (isset($data["results"]["deck"])) {
                    $descrizione = $data["results"]["deck"];
                    // Limita la descrizione a circa 200 caratteri
                    if (strlen($descrizione) > 200) {
                        $descrizione = substr($descrizione, 0, 197) . '...';
                    }
